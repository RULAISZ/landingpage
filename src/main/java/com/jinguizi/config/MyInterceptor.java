...
import com.jinguizi.utils.CheckResult;
import com.jinguizi.utils.JwtUtil;
import com.jinguizi.utils.SystemConstant;
...
public class MyInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (request.getRequestURI().contains("randomLoading")){
            return true;
        }
        if (request.getRequestURI().contains("saveBehavior")){
            return true;
        }
        // 1.获取token
